...
| **LLMs** | Groq / Llama-3.3-70b (all LLM calls route through Groq — resume parsing, JD validation, email drafting, email evaluation) |
...
```
PDF Upload → pypdf text extraction → Groq/Llama prompt → Structured JSON
Output: { roles: [{title, years_exp}], skills: [], industry, location }
```
...
- Extracts: `isValidRange`, `companyName`, `teamName`, `requiredExperience`, `reason`
- Strict experience matching rules (Senior roles need 4+ years, etc.)
- Only jobs with `isValidRange: true` are returned
...
| **POC validation** | No filtering of former employees from POC results; relies on Serper snippet freshness. |
| **LLM function naming** | `_call_gemini_json()` in `evals.py` is misnamed — it actually calls **Groq API** with Llama-3.3-70b. |
